 ...
 * @Description : 
 * @author johb
 * @time 2016年11月23日 下午2:50:23
 */
public class StatementUtil_02 {
	...
	public static String complexSqlFromatByManyTable(Map<String, Object> pMap) {
		...
		// 拼接select
		builder.append("SELECT ");
		if (pMap.containsKey("select") && null != pMap.get("select")) {
			List<String> attributes = (List<String>) pMap.get("select");
			int intSize = attributes.size();
			if (intSize > 1) {
				for (int i = 0; i < intSize - 1; i++) {
					builder.append(attributes.get(i)).append(",");
		...
		Map<String, Object> fromMap = (Map<String, Object>)pMap.get("from");
		if (fromMap.containsKey("across") && null != fromMap.get("across")) {
			List<String> acrossList = (List<String>)fromMap.get("across");
			int intAcroossSize = acrossList.size();
			int count = 0;
			for (String string : acrossList) {
				count ++;
				if (count == intAcroossSize) {
		...
		if (fromMap.containsKey("inner") && null != fromMap.get("inner")) {
			Map<String, String> tableMap = (Map<String, String>)fromMap.get("inner");
			Iterator<String> iterator = tableMap.keySet().iterator();
			while (iterator.hasNext()) {
				String key = iterator.next();
				String value = tableMap.get(key);
				builder.append(" inner join ").append(key).append(" on ").append(value);
			}
		}
		if (fromMap.containsKey("left") && null != fromMap.get("left")) {
			Map<String, String> tableMap = (Map<String, String>)fromMap.get("left");
			Iterator<String> iterator = tableMap.keySet().iterator();
			while (iterator.hasNext()) {
				String key = iterator.next();
				String value = tableMap.get(key);
				builder.append(" left join ").append(key).append(" on ").append(value);
			}
		}
		if (fromMap.containsKey("right") && null != fromMap.get("right")) {
			Map<String, String> tableMap = (Map<String, String>)fromMap.get("right");
			Iterator<String> iterator = tableMap.keySet().iterator();
			while (iterator.hasNext()) {
				String key = iterator.next();
				String value = tableMap.get(key);
				builder.append(" right join ").append(key).append(" on ").append(value);
			}
		}
		if (fromMap.containsKey("full") && null != fromMap.get("full")) {
			Map<String, String> tableMap = (Map<String, String>)fromMap.get("full");
			Iterator<String> iterator = tableMap.keySet().iterator();
			while (iterator.hasNext()) {
				String key = iterator.next();
				String value = tableMap.get(key);
				builder.append(" full join ").append(key).append(" on ").append(value);
			}
		...
		if (pMap.containsKey("where") && null != pMap.get("where")) {
			...
			if (null != params && !params.isEmpty() && params.size() != 0) {
				...
				while (iterator.hasNext()) {
					...
					/*} else {
						builder.append(key).append(value).append(" AND ");
					}*/
				}
			}
		}
		...
		if (pMap.containsKey("groups") && null != pMap.get("groups")) {
			Map<String, String> groupMap = (Map<String, String>) pMap
					.get("groups");
